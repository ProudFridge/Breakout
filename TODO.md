### TODO
- Make the blocks change colors as they lose health
- Fix the generate_grid function not being able to generate large grids of blocks
- Add the possibility for larger levels, by adding a camera to the player and changing the player's and the ball's spawn point with code
- Create a simple gameplay loop
	- Let the player be able to score points
	- Make it possible for the player to lose points
	- Add a simple main menu
- Add simple sound effects
- Add a sound track

### Maybe:
- Create different level types: infinite, round, moving blocks, etc
- Make some blocks give power-ups when destroyed
	- Make blocks with power-ups be aesthetically different
	- In the infinite level, make the chance of a power-up block spawning increase as the game goes on
	- Examples of power-ups:
		- Permanent damage buff
		- Permanent speed buff
		- etc etc, i'm sure i'll think of some other stuff later
